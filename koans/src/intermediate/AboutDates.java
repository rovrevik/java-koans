...
public class AboutDates {
    ...
    public void usingRollToChangeDatesDoesntWrapOtherFields() {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.roll(Calendar.MONTH, 12); // rolling 12 months is a long way of doing nothing
        assertEquals(cal.getTime().toString(), "Sat Mar 03 08:33:21 EST 1973");
    }

    @Koan
    public void usingDateFormatToFormatDate() {
        String formattedDate = DateFormat.getDateInstance().format(date); // default is DateFormat.MEDIUM
        assertEquals(formattedDate, "Mar 3, 1973");
    }

    @Koan
    public void usingDateFormatToFormatDateShort() {
        String formattedDate = DateFormat.getDateInstance(DateFormat.SHORT).format(date); // SHORT is numeric
